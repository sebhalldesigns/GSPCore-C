...
#include <GSPCore/Window/Platforms/Wayland/xdg-shell-client-header.h>
#include <GSPCore/Window/Platforms/Wayland/kde-server-decoration.h>
...
int GWaylandWindowManager_RunLoop(GApplication* app) {

    application = app;

    // needs a render for some reason
    if (app->rootWindow != NULL) {
        GRenderManager_RenderWindow(app->rootWindow);
    }
...
void GWaylandWindowManager_TopLevelConfigureCallback(void* data, struct xdg_toplevel* toplevel, int32_t width, int32_t height, struct wl_array* states) {
    ...
    if (application != NULL && application->rootWindow != NULL) {
        application->rootWindow->frame.size.width = width;
        application->rootWindow->frame.size.height = height;
    }
